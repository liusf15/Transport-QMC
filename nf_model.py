...
from jax.scipy.special import betainc, ndtri, ndtr
...
class Copula_NF():
    ...
    def pack_params(self):
        return jnp.concatenate([self.mu, self.L.flatten(), self.weights_.flatten()])
    ...
    def forward_and_logdet(self, x, link_func=['sigmoid', 'logit']):
        """
        x: (n, d)
        link_func: function that maps R to [0, 1] and [0, 1] to the desired range
        """
        t_x = self.mu + x @ self.L.T
        log_det = jnp.log(det(self.L @ self.L.T)) / 2
        ...
        for j in range(self.d):
            weight_j = softmax(self.weights_[j])
            output.append(bernstein_poly(t_x[:, j:j+1], deg=self.max_deg, weights=weight_j))
            log_det += jnp.log(bernstein_poly_grad(t_x[:, j:j+1], deg=self.max_deg, weights=weight_j))
        output = jnp.stack(output, axis=1)

        if link_func[1] == 'ndtri':
            # use \Phi^{-1} to map [0, 1] to R
            output = ndtri(output)
            log_det += jnp.sum(+.5 * output**2 + .5 * jnp.log(2 * jnp.pi), axis=1)

        elif link_func[1] == 'logit':
    ...
    def reverse_kld(self, params, x):
        """
        x: (num_samples, d)
        """
        self.set_params(params)
        log_q = -0.5 * jnp.sum(x**2, axis=-1) - 0.5 * self.d * jnp.log(2 * jnp.pi)
        z, log_det = self.forward_and_logdet(x)
...
def train(nf, rng, rqmc=True, max_iter=100, lr=None, nsample=2**10, fix_samples=True):
    d = nf.target.d
    params = nf.pack_params()
    optimizer = optax.lbfgs(learning_rate=lr)
    opt_state = optimizer.init(params)

    @jax.jit
    def step(params, opt_state, x):
        loss, grad = jax.value_and_grad(nf.reverse_kld)(params, x)
        updates, opt_state = optimizer.update(grad, opt_state, params, value=loss, grad=grad, value_fn=lambda params: nf.reverse_kld(params, x))
        params = optax.apply_updates(params, updates)
        return params, opt_state, loss
    ...
    if rqmc:
        ...
        def sample(n):
            return ndtri(soboleng.random(n) * (1 - MACHINE_EPSILON) + .5 * MACHINE_EPSILON)
    else:
        sample = lambda n: rng.standard_normal((n, d))
    if fix_samples:
        x = sample(nsample)
    losses = []
    pbar = trange(max_iter)
